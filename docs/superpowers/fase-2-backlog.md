# Backlog para a Fase 2 (itens adiados nas revisões da Fase 1)

Nenhum destes bloqueia a Fase 1. São melhorias a encaixar quando a Fase 2
(CRUD de cardápio) mexer nas áreas relacionadas.

## Segurança / front-end
- `frontend/app.js`: trocar interpolação em `innerHTML` por `textContent` /
...
- `backend/app/routers/pedidos.py`: `raise HTTPException(...) from erro`
  (preserva a causa; silencia ruff B904).
- `backend/app/routers/produtos.py`: remover a anotação `-> list` "pelada".
- `backend/app/schemas.py`: `field_validator` que dá `strip()` e revalida
...
- `backend/app/database.py`: tirar o `DB_DIR.mkdir(...)` do import; chamar
...
## Processo
- Padronizar o trailer `Co-Authored-By` nos commits a partir da Fase 2.
